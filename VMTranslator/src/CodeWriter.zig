...
const BaseAddressMap = @import("map/BaseAddressMap.zig");
const CommandType = @import("map/CommandMap.zig").Type;
const Operation = @import("map/OperationMap.zig").Type;
const Segment = @import("map/SegmentMap.zig").Type;
...
pub fn writeArithmetic(self: *Self, operation: Operation) !void {
...
fn writePush(self: *Self, segment: Segment, index: usize) !void {
...
fn writePop(self: *Self, segment: Segment, index: usize) !void {
    var buf: []u8 = undefined;
    switch (segment) {
        .Argument, .Local, .That, .This => {
            const pAddr: u8 = switch (segment) {
...
test "smoke" {
    var cw = try init(testing.allocator, testing.io, "test/Test.asm");
    defer cw.deinit();
...
test "setFileName and close" {
    ...
    var cw = try init(testing.allocator, testing.io, outputPath);
    defer cw.deinit();
